...
class Admin extends CI_Controller {
    ...
    {   
       ...
       $this->form_validation->set_rules('state', 'State', 'required|is_unique[mst_state.state]',array('is_unique' => 'This %s Alrady Exist!'));
        ...
        { 
            ...
            if($insertFlag)
            {
                $result = array('status'=>true,'msg'=>'New State Added!');
            }
            else
            {
                $result = array('status'=>true,'msg'=>'Unable to Add New State Something Went Wrong Please Try Again!');            
            }
        }
    ...
    {   
        ...
        if($state_id)
        {
           $this->form_validation->set_rules('state', 'State', 'required|callback_uniquestate['.$state_id.']',array('uniquestate' => 'This %s Alrady Exist!'));
            ...
            { 
                ...
                if($insertFlag)
                {
                   $result = array('status'=>true,'msg'=>'State Name Updated!');
                }
                else
                {
        ...
        {
            $result = array('status'=>false,'msg'=>'State Id Blank!');
        }
        echo json_encode($result);
    }
