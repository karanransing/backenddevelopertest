  ...
  <div class="row">
              ...
              <div class="table-responsive">
                <table id="datatableUser" class="table table-striped table-bordered">
                  <thead>
                    ...
                    <tr>
                      <th width="50%"><button type="button" class="form-control btn btn-info submit_new"><?php if(!empty($city_data)){ echo "Update";} else { echo "Add";}?></button></th>
    ...
    <script>
      ...
      $('.submit_new').click(function(){ 
        if($("#add_city").valid())
        { 
          var dataString = $("#add_city").serialize();
          var url = $('#add_city').attr('action');
          $.ajax({
            type:"POST",
            url:url,
            data:dataString,
            ...
            success:function (result) {
              ...
              if(result.status)
              {
                toastr.success(result.msg, 'City', { "progressBar": true });
              }
              else
              { 
                toastr.error(result.msg, 'City', { "progressBar": true });
              }
            },
            error:function()
            {
              toastr.error('Ajax Call Error', 'City', { "progressBar": true });
            }
          });
        }
      });
